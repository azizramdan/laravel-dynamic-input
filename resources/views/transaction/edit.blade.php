...
@endif
<form action="/{{ $transaction->id }}" id="edit-form" method="post">
    @csrf
    @method('patch')
    <div class="mb-3">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="customer_name" name="customer_name" value="{{ old('customer_name') ?? $transaction->customer_name }}" required>
    </div>
    <div class="mb-3">
        <label for="status" class="form-label">Payment</label>
        <select class="form-select" name="status">
            <option value="cash" {{ (old('status') ?? $transaction->status) == 'cash' ? 'selected' : '' }}>Cash</option>
            <option value="credit" {{ (old('status') ?? $transaction->status) == 'credit' ? 'selected' : '' }}>Credit</option>
        </select>
    </div>
...
@push('script')
    <script>
        ...
        if (oldValues.categoryId.length) {
            ...
            items.forEach(value => {
                addItem(value.pivot.category_id, value.pivot.name, value.pivot.qty, value.pivot.unit, value.pivot.price)
            })
        }
        ...
        $('#edit-form').on('submit', function (e) {
            ...
            $('#edit-form select[name="category_id[]"]').each(function () {
